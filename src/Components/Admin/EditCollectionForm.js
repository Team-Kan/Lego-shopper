...
const EditCollectionForm = (props) => {
  const { showAllCollections, collection, setEditCollection, setIsLoading } =
    props;
  ...
  const handleEdit = async (ev) => {
    ...
    const editCollection = await editCollectionFetch(
      token,
      name,
      collection.id,
    );
  ...
  useEffect(() => {
    collection ? setName(collection.name || "") : setName("");
  }, [collection]);
  return (
    <form
    ...
    >
      ...
      />
      <div>{error}</div>
      <button className="pl-4 pr-4 mb-4 bg-green-600 rounded-lg shadow-md shadow-green-700 hover:animate-pulse active:bg-green-800 active:animate-none active:translate-y-1">
        Edit
      </button>
      <button
