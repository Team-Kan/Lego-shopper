...
describe("CreateCart(userId)", () => {
  it("should create an active cart for user if one is not avaible", async () => {
    const {id, username} = await createUser({username: "bob", password: "bob_password"})
    ...
    expect(response.body).toEqual(expect.objectContaining({id: 1, userId: 1}))
  })

  it("should return a an active cart for user if one exist", async () => {
    const token = await authenticate({username:"bob", password: "bob_password"});
    const response = await request(app).get(
      `/api/cart/`
    )
    .set("Authorization", `Bearer ${token}`);
